...
Security scan stack:
- **Trivy**: `trivy config --tf-vars environments/<env>/<stack>/terraform.tfvars environments/<env>/<stack>`
- **Checkov**: `checkov -d environments/<env>/<stack> --framework terraform --config-file .checkov.yml`
- **conftest**: `conftest test tfplan.json -p policy/` (OPA policies)

Stack values: `<stack>` = `base` / `data` / `app`
